...
comet = universe.Planet("Comet", 1e14, np.array([4*au, 4*au]), np.array([-20000, -15000]), "green")
...
solar_system.add_planets(comet)

x, y = solar_system.evolve(year, 36000)
...
line5, = ax.plot([], [], 'o',color = comet.color)
...
def animate(i):
    ...
    line5.set_data(xcomet[i], ycomet[i])
...
# def target():
#     for planet in solar_system.add_planets:
#         d = math.sqrt((planet.y-y[i])**2+(planet.x-x[i])**2)
#         while d <= 0:
